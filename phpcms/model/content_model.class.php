...
class content_model extends model
{
    ...
    private $url;
    private $siteid;
    /** @var search_model $search_db */
    private $search_db;
    private $fields;
    private $hits_db;
    private $content_check_db;
    private $attachment_db;
    private $message_db;
    /** @var category_model $category_db */
    private $category_db;
    ...
    public function add_content($data, $isimport = 0)
    {
        ...
        if (isset($_POST['auto_thumb']) && $systeminfo['thumb'] == '' && isset($modelinfo['content'])) {
            $content = (isset($content) && $content) ? $content : stripslashes($modelinfo['content']);
        ...
        if (!$isimport && $data['status'] == 99) {
            ...
            if (defined('RELATION_HTML') && isset($catid)) $html->create_relation_html($catid);
        }
        return $id;
    }
    ...
    public function edit_content($data, $id)
    {
        ...
        if (!defined('IN_ADMIN')) {
            ...
            $user = $this->get_one(array( 'id' => $id, 'username' => $_username ));
        ...
        // 生成静态
        $html = pc_base::load_app_class('html', 'content');
        if (isset($urls) && $urls['content_ishtml']) {
    ...
    /**
     * 批量更新内容状态
     * @param array $ids 需要更新的内容ID
     * @param int $status 更新到状态
     * @return bool
     */
    public function status($ids = array(), $status = 99)
    {
        ...
        if (is_array($ids) && !empty($ids)) {
            foreach ($ids as $id) {
                ...
                if ($status == 0) {
                    // 退稿发送短消息和邮件
                    $message = L('reject_message_tips') . $r['title'] . "<br/><a href=\'index.php?m=member&c=content&a=edit&catid={$r['catid']}&id={$r['id']}\'><span style='color: red;'>" . L('click_edit') . "</span></a><br>";
    ...
    /**
     * 获取单篇信息
     * @param string|integer $catid 分类ID
     * @param string|integer $id 内容ID
     * @return array|bool
     */
