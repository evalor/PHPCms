 ...
 * @param integer $isjs 是否执行字符串格式化，默认为执行
 ...
 * @param string $chars 可选 默认为0123456789
 * @return string 字符串
 */
 ...
 * @return array|mixed|string 返回数组格式 如果 data为空 则返回空数组
 */
...
function my_error_handler($errno, $errstr, $errfile, $errline)
{
    ...
    if (pc_base::load_config('system', 'errorlog')) {
        ...
        $str = '<div style="font-size:12px;text-align:left; border-bottom:1px solid #9cc9e0; border-right:1px solid #9cc9e0;padding:1px 4px;color:#000000;font-family:Arial, Helvetica,sans-serif;"><span>errorno:' . $errno . ',str:' . $errstr . ',file:<span style="color: blue">' . $errfile . '</span>,line' . $errline . '<br /><a href="http://faq.phpcms.cn/?type=file&errno=' . $errno . '&errstr=' . urlencode($errstr) . '&errfile=' . urlencode($errfile) . '&errline=' . $errline . '" target="_blank" style="color:red">Need Help?</a></span></div>';
        echo $str;
    }
    return '';
}
 ...
 * @example showmessage('登录成功', array('默认跳转地址'=>'http://www.phpcms.cn'));
...
{
    extract([ 'msg' => $msg, 'url_forward' => $url_forward, 'ms' => $ms, 'dialog' => $dialog, 'returnjs' => $returnjs ]);
    if (defined('IN_ADMIN')) {
 ...
 * @param string $filepath 数据路径（模块名称）
 * @param string $type 缓存类型[file,memcache]
 ...
 * @param string|array $data 条件数组或者字符串
 * @param string $front 连接符
 * @param string $in_column 字段名称
 * @return string
 */
function to_sqls($data, $front = ' AND ', $in_column = '')
{
...
/**
 * 分页函数
 * @param integer $num 信息总数
 * @param integer $curr_page 当前分页
...
/**
 * 代码广告展示函数
 * @param integer $siteid 所属站点
 * @param integer $id 广告ID
 * @return bool|mixed|string 返回广告代码
 */
 ...
 * @param integer $uid 默认为phpssouid
...
/**
 * 通过id获取显示联动菜单
 * @param integer $linkageid 联动菜单ID
 * @param integer $keyid 菜单keyid
 ...
 * @param integer|null $siteid 站点id，系统需根据站点id获取水印信息
 * @return string 原图片路径 生成水印图片途径 默认为空 覆盖原图
 */
function watermark($source, $target = '', $siteid = null)
{
    global $image_w;
...
{
    extract([ 'time_start' => getmicrotime() ]);
    $data = '';
    $getpclass = FALSE;
