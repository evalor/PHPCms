 ...
 * @param array $diyarr 数据包
 * @param int $isall 是否显示全部
 * @return mixed
 */
...
function video_filters_sql($modelid, $catid)
{
    ...
    if ($category[$catid]['child']) {
        $sql .= ' AND `catid` IN(' . $category[$catid]['arrchildid'] . ')';
    } else {
        $sql .= ' AND `catid`=\'' . $catid . '\'';
    }
...
function player_code($id = 'video_player', $channelid = '', $vid = '', $width = 622, $height = 460, $style_projectid = '')
{
    ...
    $string = '<embed id="' . $id . '" name="' . $id . '" src="' . $playerurl . '" width="' . $width . '" height="' . $height . '" quality="high" align="middle" allowScriptAccess="always" allowfullscreen="true" type="application/x-shockwave-flash">';
    return $string;
}
